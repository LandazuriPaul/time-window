...
func (r Result) GithubFormat() string {
	return fmt.Sprintf("error=%s\nis_valid=%t\nmessage=%s\ntimestamp=%d\nresult=%s\n", r.Error, r.IsValid, r.Message, r.Timestamp, r)
}
...
func (r Result) String() string {
	...
	if err != nil {
		...
		return fmt.Sprintf("result={\"error\":\"%s\",\"isValid\":%t,\"message\":\"%s\",\"timestamp\":%d,\"marshallingError\":\"%s\"}", r.Error, r.IsValid, r.Message, r.Timestamp, err)
	}
	return string(b)
}
